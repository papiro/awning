...
module.exports = function beans (req, res, done) {
  ...
  fs.readFile(bnsPath, { encoding: 'utf8' }, (err, markup) => {
    ...
    beansUtil.forEachBean({ markup, context: req.context }, beanData => {
      let output
      if (beanData.fn) {
        output = 
        `
        <script id="precompiledBeans" type="text/javascript">
        alert('foo')
        ...
        `
        output = '<script id="foo">alert("foo")</script>'
      } else {
        const { markup } = beanData
        debug(`Writing bean:::${markup}`)
        output = markup
      }
      res.write(output)
    })
