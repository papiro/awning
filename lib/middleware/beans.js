...
,
  { Beans } = require('./utils/beans-core')
;
...
module.exports = function beans (req, res, done) {
  ...
  //now onto business
  const bnsPath = path.join(res.root, req.url)
  ...
  fs.readFile(bnsPath, { encoding: 'utf8' }, (err, markup) => {
    ...
    const beans = new Beans(markup)

    const output = beans.evaluate(req.context)

    debug('bean became ', output)
    res.writeHead(200, {
