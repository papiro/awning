...
module.exports = {
  forEachBean ({ markup, context }, cb) {
    let match, noBeans = true, previousMatchEnd = 0
    ...
    while ((match = masterRegEx.exec(markup)) !== null) {
      ...
      // Now get the actual match from the array.
      match = match[2].trim()

      debug(`Bean ID:::${beanId}`)
      debug(`Raw match:::${match}`)

      if (beanId) {
        const precompiled = precompile(match, beanId)
...
function propDescLookup (obj, desc) {
  return desc.split('.').reduce((obj, key) => {
    return obj[key] 
  }, obj)
}
...
function precompile (bean, beanId) {
  ...
  return `function beanFn (collection) {
    if (Array.isArray(collection)) {
      return collection.map(${context_internal} => \`${markup}\`).join('')
    } else if (typeof collection === 'object') {
      return Object.keys(collection).map(${context_internal} => \`${markup}\`).join('')
    }
  }`
}
