...
class Bean {
  constructor (bean) {
    ...
    if (exp[0] === '[') {
      ...
      return context.map( val => markup.replace(new RegExp('\\$\\{'+varName+'\\}'), val)).join('')
  ...
  compile () {
    ...
    if (this.exp[0] === '[') {
      ...
      context = contextStr
                ...
                .map( val => val.replace(/^['"]|['"]$/g, ''))
      // interpolate the array values into the expanded markup
      this.result = context.map( val => this.markup.replace(new RegExp('\\$\\{'+varName+'\\}'), val)).join('')
